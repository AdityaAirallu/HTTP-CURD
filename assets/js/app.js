...
const onDeleteHanlder =(ele) => {
    cl(ele)
    let getDeleteId = +ele.dataset.id;
    let deletUrl = `${baseUrl}/ ${getDeleteId}`;
    makeNetworkCall(`DELETE`, deletUrl);
...
function templating(arr) {
    ...
    arr.forEach((ele,i) => {
        ...
        <tr>
            ...
            <td>${ele.title}</td>
            <td>${ele.body}</td>
            <td><button data-id='${ele.id}' class="btn btn-info" onclick="onEditHanlder(this)">Edit</button></td>
            <td><button data-id='${ele.id}' class="btn btn-danger" onclick="onDeleteHanlder(this)">Delete</button></td> 
            </tr>
        `;
    });
